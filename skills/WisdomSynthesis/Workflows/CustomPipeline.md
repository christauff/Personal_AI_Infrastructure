...
### Step 0a: Detect Content Input Type

**CRITICAL:** Check if user provided file/content or just skill sequence.

**Input Detection:**
```bash
# Check for file path in user input
if [[ "$INPUT" =~ \.txt|\.md|\.pdf|^~/ ]]; then
  HAS_FILE=true
  echo "📄 File detected - will skip Research if it's in the pipeline"
fi
```

**⚠️ RESOURCE WARNING:**
- If pipeline includes Research: May spawn 4-12 agents depending on mode
- File mode: Recommend removing Research from pipeline

---

### Step 0b: Parse User's Skill Sequence
...
- If file provided AND pipeline starts with Research → **suggest removing Research step**
- Example: "Research → Fabric → Council" with file → "Fabric → Council" instead
...
```yaml
# ~/.claude/skills/PAI/USER/SKILLCUSTOMIZATIONS/WisdomSynthesis/Pipelines.yaml
