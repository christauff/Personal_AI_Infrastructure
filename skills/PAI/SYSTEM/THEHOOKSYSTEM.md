...
- **Inference:** `import { inference } from '../skills/PAI/Tools/Inference'` → `inference({ level: 'standard', expectJson: true })`
...
- After receiving Task result, parent calls: `bun ~/.claude/skills/PAI/Tools/UpdateTabTitle.ts "Message"`
- This avoids transcript parsing race conditions
- See `~/.claude/skills/PAI/Tools/UpdateTabTitle.ts` for implementation
...
Configure voice IDs in `skills/PAI/SYSTEM/AGENTPERSONALITIES.md` or via environment variables.
...
**Full Documentation:** See `~/.claude/skills/PAI/SYSTEM/TERMINALTABS.md`
...
1. **Added CAPTURE field to response format** (see `~/.claude/skills/PAI/SKILL.md`)
...
**Tracking:** Documented in `~/.claude/skills/PAI/SKILL.md` (History Capture System section)
...
- **Agent System:** `~/.claude/skills/PAI/SYSTEM/AGENTPERSONALITIES.md`
- **History/Memory:** `~/.claude/skills/PAI/SYSTEM/MEMORYSYSTEM.md`
