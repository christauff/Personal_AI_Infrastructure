...
### Naming Convention: Deliberate Divergence from Anthropic

PAI uses **TitleCase** (e.g., `Browser`, `CreateSkill`). Anthropic's official guide mandates **kebab-case** (e.g., `browser`, `create-skill`).

**Why PAI diverges:**
1. TitleCase maps naturally to TypeScript class/module naming conventions used in `Tools/`
2. PAI predates Anthropic's naming guide and has 47+ skills in TitleCase
3. Claude Code's skill loader reads SKILL.md regardless of folder casing -- no enforcement exists
4. Personal skills (`_ALLCAPS`) have no Anthropic equivalent

**Risk:** If Anthropic ever enforces kebab-case at the platform level, PAI skills would need renaming. Current risk: LOW (no enforcement exists as of Feb 2026).

**Interop strategy:** If distributing skills via Anthropic's ecosystem, add a `standard_name` field to frontmatter:
```yaml
---
name: CreateSkill
standard_name: create-skill    # Anthropic-compatible kebab-case alias
---
```
...
- **NO XML angle brackets (`<` or `>`) in frontmatter** — frontmatter loads into system prompt; angle brackets are reserved and could enable prompt injection
- **NO "claude" or "anthropic" in skill name** — reserved by Anthropic

### Optional Metadata Fields

Skills MAY include additional YAML frontmatter fields for distribution and management:

```yaml
---
name: SkillName
description: What it does. USE WHEN [triggers]. Do NOT use for [negative triggers].
compatibility: "Requires bun, Linux/macOS. Network access needed for API calls."
metadata:
  author: PAI
  version: 1.0.0
license: MIT
...
|-------|---------|-------------|
| `compatibility` | Environment requirements (OS, runtime, network) | Skills with external dependencies |
| `metadata.author` | Who created the skill | All skills (for PAI Pack distribution) |
| `metadata.version` | Semantic version | Skills that evolve across releases |
| `license` | Open-source license (MIT, Apache-2.0) | Skills shared via PAI Packs |

**Note:** These fields are optional for internal use but recommended for any skill intended for distribution.
...
**Keep SKILL.md under 5,000 words.** This is a hard limit per Anthropic's recommendations.

- SKILL.md loads into context on every skill invocation — oversized files waste tokens
- Move detailed documentation to context files in skill root (loaded on-demand)
- Use the Dynamic Loading Pattern (below) for skills exceeding 100 lines
- To check: `wc -w SKILL.md` — must be under 5,000

**If a SKILL.md exceeds 5,000 words**, refactor using the Dynamic Loading Pattern:
1. Keep routing, examples, and quick reference in SKILL.md
2. Move detailed docs to separate `.md` files in skill root
3. Reference via `SkillSearch()` or direct `Read`

### Negative Triggers (Recommended for High-Traffic Skills)

**For skills that may collide with other skills, add negative triggers to the description.**

Pattern: `Do NOT use for [specific case] (use [other-skill] instead).`

This reduces false activation when 20+ skills share similar trigger words. Add negative triggers when:
- Two or more skills share trigger keywords (e.g., "research" appears in Research, OSINT, and Fabric)
- A skill frequently activates for requests meant for another skill
- Users report unexpected skill invocations
...
### Context Files: Flat Root vs Subdirectory (Deliberate Choice)

Anthropic's guide recommends a `references/` subdirectory for on-demand documentation. PAI uses **flat root files** instead. This is a deliberate architectural choice:

**Why flat root:**
1. Fewer directory levels = faster discovery with `ls` and `Glob`
2. All skill content is visible at one level -- no hidden docs in subdirectories
3. Consistent with PAI's "maximum 2 levels deep" flat structure rule
4. Context files ARE the skill, not secondary artifacts

**Anthropic's `references/` approach** is valid for skills distributed as packages where users need clear separation between "what to read" and "what runs." PAI's internal skills don't need this separation.
...
- [ ] **No XML angle brackets (`<` or `>`) in frontmatter** (security: prompt injection vector)
- [ ] **No "claude" or "anthropic" in skill name** (reserved by Anthropic)
- [ ] **SKILL.md under 5,000 words** (`wc -w SKILL.md`)
- [ ] Negative triggers added if skill collides with others
...
**See:** `~/.claude/skills/PAI/SYSTEM/CLIFIRSTARCHITECTURE.md` (Workflow-to-Tool Integration section)
...
### Error Recovery in Workflows (Recommended)

Workflows that modify external state (deploy, publish, post, delete) SHOULD include an `## Error Recovery` section with explicit rollback steps:

```markdown
## Error Recovery

If Step 3 (deploy) fails:
1. Revert: `git checkout main -- public/`
2. Rebuild: `bun run build`
3. Verify local state is clean before retrying

If Step 4 (DNS update) fails:
1. Previous deployment is still live (no rollback needed)
2. Check DNS propagation: `dig example.com`
3. Retry after 5 minutes
```

**When to add Error Recovery:**
- Workflows that deploy to production
- Workflows that post to external APIs (Twitter, GitHub)
- Workflows that modify shared state (databases, config files)
- Workflows that create irreversible changes
...
**See:** `~/.claude/skills/PAI/SYSTEM/CLIFIRSTARCHITECTURE.md` (Configuration Flags section) for full documentation
...
**Measure skill effectiveness using the Evals framework.**

Skills should demonstrably improve task completion vs unassisted Claude. Track:

| Metric | What It Measures | Target |
|--------|-----------------|--------|
| Token consumption | Total tokens with vs without skill | Equal or fewer with skill |
| Message count | Back-and-forth needed | Fewer with skill |
| Routing accuracy | Correct skill activates | 90%+ on TriggerTests.md phrases |
| First-try success | No user correction needed | Higher with skill |
| Failed tool calls | API/tool errors per workflow | Zero |

**How to measure:**
1. Run representative tasks WITH the skill enabled
2. Run same tasks WITHOUT the skill (direct Claude)
3. Compare metrics across 3+ trials
4. Track over time via `Evals/Data/SkillPerformanceSuite.yaml`

**When to measure:**
- After significant skill changes (new workflows, trigger updates)
- Weekly for high-traffic skills (Research, Browser, Art)
- Before/after adding negative triggers (to verify collision reduction)

**Eval suite:** `bun run ~/.claude/skills/Evals/Tools/AlgorithmBridge.ts -s skill-performance`

---

## Performance Notes (Recommended for Complex Workflows)

For workflows with many steps or validation requirements, add a Performance Notes section to counter model laziness:

```markdown
## Performance Notes
- Complete ALL steps in order -- do not skip validation
- Quality is more important than speed
- If a step fails, investigate before proceeding
- Do not summarize or abbreviate output unless explicitly asked
```

**When to add:** Multi-step workflows (5+ steps), workflows with validation gates, workflows producing user-facing output. Anthropic research shows this is more effective in user prompts than in SKILL.md, but having it documented ensures consistency.
...
PAI skills are local-first (files on disk), but Anthropic also offers a cloud Skills API for programmatic management. This section documents the API for future integration.

### API Endpoints (`/v1/skills`)

| Operation | Method | Endpoint | Notes |
|-----------|--------|----------|-------|
| Create | POST | `/v1/skills` | Upload SKILL.md files, set `display_title` |
| List | GET | `/v1/skills` | Pagination, filter by `source` (custom/anthropic) |
| Get | GET | `/v1/skills/{skill_id}` | Returns metadata, `latest_version_id` |
| Delete | DELETE | `/v1/skills/{skill_id}` | Removes skill |
| Create Version | POST | `/v1/skills/{skill_id}/versions` | Version management |
| List Versions | GET | `/v1/skills/{skill_id}/versions` | Version history |

**Beta header required:** `anthropic-beta: skills-2025-10-02`

### Messages API Integration (`container.skills`)

Skills attach to Messages API requests via the `container` parameter:
...
- **Container IDs reusable** across requests for state persistence
- **Two skill types:** `anthropic` (pre-built: pptx, xlsx, docx, pdf) and `custom` (user-uploaded)

### SDK Support

| SDK | List | Create | Example |
|-----|------|--------|---------|
| Python | `client.beta.skills.list(source="anthropic")` | `client.beta.skills.create(...)` | Official |
| TypeScript | `client.beta.skills.list({source: "anthropic"})` | Similar | Official |
| LiteLLM | `list_skills()` / `alist_skills()` | `create_skill()` / `acreate_skill()` | Proxy with cost tracking |

### Open Standard (December 18, 2025)

- Published at **agentskills.io** as portable specification
- GitHub: `anthropics/skills` (70k+ stars, 20+ example skills)
- Organization-wide deployment for Team/Enterprise plans
- Skills portable across Claude Code, Claude.ai, and API

### PAI Integration Status

PAI uses local skills (47 skills on disk). Future integration points:

1. **Publish PAI skills to API** — Upload custom skills for cloud access
2. **Consume Anthropic pre-built skills** — Use pptx/xlsx/docx/pdf via API
3. **Interop via `standard_name`** — Map PAI TitleCase names to kebab-case API names
4. **Organization deployment** — Deploy PAI skills across team workspaces

**No action required now** — PAI's local-first architecture works independently. API integration is additive when needed.
