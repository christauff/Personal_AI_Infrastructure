...
async function findTaskResult(transcriptPath: string, maxAttempts: number = 2): Promise<TaskResultData> {
                  ...
                  if (resultEntry.type === 'user' && resultEntry.message?.content) {
                    for (const resultContent of resultEntry.message.content) {
                      if (resultContent.type === 'tool_result' && resultContent.tool_use_id === content.id) {
                        ...
                        // Extract metrics from <usage> tag in text content (Claude Code v2.1.30+)
                        // Format: <usage>total_tokens: N\ntool_uses: N\nduration_ms: N</usage>
                        let metrics: TaskMetrics | null = null;
                        const usageMatch = taskOutput.match(/<usage>([\s\S]*?)<\/usage>/);
                        if (usageMatch) {
                          const usageText = usageMatch[1];
                          const tokenMatch = usageText.match(/total_tokens:\s*(\d+)/);
                          const toolsMatch = usageText.match(/tool_uses:\s*(\d+)/);
                          const durationMatch = usageText.match(/duration_ms:\s*(\d+)/);

                          if (tokenMatch || toolsMatch || durationMatch) {
                            metrics = {
                              token_count: tokenMatch ? parseInt(tokenMatch[1], 10) : undefined,
                              tool_uses: toolsMatch ? parseInt(toolsMatch[1], 10) : undefined,
                              duration_ms: durationMatch ? parseInt(durationMatch[1], 10) : undefined,
                            };
                          }
                        }

                        return { result: taskOutput, agentType, description, toolInput, metrics };
                      }
                    }
                  }
                }
  ...
  return { result: null, agentType: null, description: null, toolInput: null, metrics: null };
}
...
/**
 * Log Task tool metrics to MEMORY/AGENTS/metrics.jsonl
 * Captures token_count, tool_uses, duration_ms for efficiency analysis
 */
async function logTaskMetrics(
  agentType: string,
  description: string | null,
  metrics: TaskMetrics | null,
  ...
  const metricsDir = paiPath('MEMORY', 'AGENTS');
  if (!existsSync(metricsDir)) {
    mkdirSync(metricsDir, { recursive: true });
  }

  const metricsFile = join(metricsDir, 'metrics.jsonl');
  ...
  const entry = {
    ...
    tokens_per_tool: metrics.token_count && metrics.tool_uses
      ? Math.round(metrics.token_count / metrics.tool_uses)
      : null,
    ms_per_tool: metrics.duration_ms && metrics.tool_uses
      ? Math.round(metrics.duration_ms / metrics.tool_uses)
      : null,
  };

  appendFileSync(metricsFile, JSON.stringify(entry) + '\n');
  console.error(`📊 Logged metrics: ${agentType} - ${metrics.token_count} tokens, ${metrics.tool_uses} tools, ${metrics.duration_ms}ms`);
}
...
async function main() {
  ...
  const { result: taskOutput, agentType, description, toolInput, metrics } = await findTaskResult(transcriptPath);
  ...
  // Log Task tool metrics FIRST (before completion message check)
  // Metrics should always be logged if available, regardless of message format
  const prelimAgentType = agentType || instanceMetadata.agent_type || 'default';
  if (metrics) {
    debug(`📊 Metrics found: ${JSON.stringify(metrics)}`);
    try {
      const parsed = JSON.parse(input);
      await logTaskMetrics(prelimAgentType, description, metrics, parsed.session_id);
    } catch (e) {
  ...
  if (!completionMessage) {
    ...
    // Still exit - other processing (voice, capture) needs completion message
    process.exit(0);
  }
  ...
  // Note: Metrics already logged earlier (before completion message check)
  ...
  try {
    ...
    // Add Task tool metrics if available (Claude Code v2.1.30+)
    if (metrics) {
      event.metrics = {
        token_count: metrics.token_count,
        tool_uses: metrics.tool_uses,
