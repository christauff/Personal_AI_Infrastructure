...
SKILLS_CACHE="$PAI_DIR/MEMORY/STATE/active-skills.json"
...
# --- PARALLEL BLOCK START ---
{
    ...
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if [ -f "$GIT_REPO_CACHE" ]; then
            git_cache_mtime=$(stat -c %Y "$GIT_REPO_CACHE" 2>/dev/null || echo 0)
...

{
    ...
    if [ "$cache_age" -gt "$WEATHER_CACHE_TTL" ]; then
        ...
        weather_json=$(curl -s --max-time 3 "https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code&temperature_unit=fahrenheit" 2>/dev/null)
...

{
    ...
    if [ -f "$COUNTS_CACHE" ]; then
        counts_cache_mtime=$(stat -c %Y "$COUNTS_CACHE" 2>/dev/null || echo 0)
    ...
    if [ "$counts_cache_valid" = true ]; then
        ...
        GETCOUNTS_TOOL="$PAI_DIR/skills/PAI/Tools/GetCounts.ts"
...
# NOTE: DA_NAME, PAI_VERSION, input data, cc_version, model_name, dir_name
# already parsed in the initial block (lines 57-104). No duplicate parsing needed.
...
case "$MODE" in
    ...
    normal)
        printf "${SLATE_600}── │${RESET} ${PAI_P}P${PAI_A}A${PAI_I}I${RESET} ${PAI_A}STATUSLINE${RESET} ${SLATE_600}│ ──────────────────────────────────────────────────${RESET}\n"
        printf "${PAI_LABEL}LOC:${RESET} ${PAI_CITY}${location_city}${RESET}${SLATE_600},${RESET} ${PAI_STATE}${location_state}${RESET} ${SLATE_600}│${RESET} ${PAI_TIME}${current_time}${RESET} ${SLATE_600}│${RESET} ${PAI_WEATHER}${weather_str}${RESET}\n"
        printf "${SLATE_400}ENV:${RESET} ${SLATE_400}CC:${RESET} ${PAI_A}${cc_version}${RESET} ${SLATE_600}│${RESET} ${SLATE_500}PAI:${RESET} ${PAI_A}v${PAI_VERSION}${RESET} ${SLATE_600}│${RESET} ${SLATE_400}Model:${RESET} ${GREET_PRIMARY}${model_name}${RESET}\n"
        printf "${WIELD_ACCENT}Skills:${RESET} ${SLATE_300}${skills_count}${RESET} ${SLATE_600}│${RESET} ${WIELD_WORKFLOWS}Workflows:${RESET} ${SLATE_300}${workflows_count}${RESET} ${SLATE_600}│${RESET} ${WIELD_HOOKS}Hooks:${RESET} ${SLATE_300}${hooks_count}${RESET}\n"
        ;;
esac

# ═══════════════════════════════════════════════════════════════════════════════
# ACTIVE SKILLS / SKILL SUGGESTIONS (dynamic display)
# ═══════════════════════════════════════════════════════════════════════════════

SKILL_TTL=300  # 5 minutes in seconds
active_skills_display=""

if [ -f "$SKILLS_CACHE" ]; then
    now_epoch=$(date +%s)
    # Read skills from cache, filter by TTL, format for display
    active_skills_display=$(jq -r --argjson now "$now_epoch" --argjson ttl "$SKILL_TTL" '
        .skills // [] |
        map(select(($now - (.timestamp / 1000)) < $ttl)) |
        map(.skill) |
        join(" → ")
    ' "$SKILLS_CACHE" 2>/dev/null)
fi

SKILL_ACTIVE='\033[38;2;234;179;8m'    # Amber/gold for active skills
SKILL_SUGGEST='\033[38;2;139;92;246m'  # Purple for suggestions

# Show active skills OR suggestion (never both)
if [ -n "$active_skills_display" ] && [ "$active_skills_display" != "" ]; then
    # Active skills - show what's running
    case "$MODE" in
        nano|micro)
            printf "${SKILL_ACTIVE}⚡${RESET} ${SLATE_300}${active_skills_display}${RESET}\n"
            ;;
        mini|normal)
            printf "${SKILL_ACTIVE}⚡${RESET} ${SKILL_ACTIVE}ACTIVE:${RESET} ${SLATE_300}${active_skills_display}${RESET}\n"
            ;;
    esac
else
    # Skill suggestions disabled per user request
    # To re-enable, uncomment the block below
    : # no-op placeholder
    # SUGGESTIONS=(
    #     "/Research|Multi-agent web research with citations"
    #     "/RedTeam|Stress-test ideas with 32 adversarial agents"
    #     "/FirstPrinciples|Break down problems to fundamentals"
    #     "/OSINT|Background checks and due diligence"
    #     "/BeCreative|Extended thinking for creative solutions"
    #     "/Fabric|242+ analysis patterns for any content"
    #     "/Council|Multi-agent debate on complex topics"
    #     "/Browser|Visual verification and web automation"
    #     "/Recon|Security reconnaissance and attack surface"
    #     "/Art|Generate images, diagrams, and visuals"
    #     "/Telos|Life goals, projects, and dependencies"
    #     "/WisdomSynthesis|Deep multi-skill content analysis"
    # )
    #
    # hour=$(date +%H)
    # idx=$((10#$hour % ${#SUGGESTIONS[@]}))
    # suggestion="${SUGGESTIONS[$idx]}"
    # skill_name="${suggestion%%|*}"
    # skill_desc="${suggestion#*|}"
    #
    # case "$MODE" in
    #     nano)   printf "${SKILL_SUGGEST}💡${RESET} ${SLATE_300}${skill_name}${RESET}\n" ;;
    #     micro)  printf "${SKILL_SUGGEST}💡${RESET} ${SLATE_300}${skill_name}${RESET}\n" ;;
    #     mini)   printf "${SKILL_SUGGEST}💡${RESET} ${SKILL_SUGGEST}TRY:${RESET} ${SLATE_300}${skill_name} - ${skill_desc}${RESET}\n" ;;
    #     normal) printf "${SKILL_SUGGEST}💡${RESET} ${SKILL_SUGGEST}TRY:${RESET} ${SLATE_300}${skill_name} - ${skill_desc}${RESET}\n" ;;
    # esac
fi
...
if [ -f "$RATINGS_FILE" ] && [ -s "$RATINGS_FILE" ]; then
    ...
    if [ -f "$LEARNING_CACHE" ]; then
        cache_mtime=$(stat -c %Y "$LEARNING_CACHE" 2>/dev/null || echo 0)
        ratings_mtime=$(stat -c %Y "$RATINGS_FILE" 2>/dev/null || echo 0)
